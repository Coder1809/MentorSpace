          ...
          filteredMentors.map((mentor) => (
            ...
            >
              <CardContent className="p-6 space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    {mentor.profilePhoto ? (
                      <img
                        ...
                        alt={mentor.name}
                        className="w-14 h-14 rounded-2xl object-cover border border-[#E5E7EB] shadow-md shrink-0"
                      />
                    ) : (
                      <div className="w-14 h-14 rounded-2xl bg-[#4CAF7D] text-white flex items-center justify-center font-extrabold text-xl shadow-md shadow-[#4CAF7D]/20 shrink-0">
                        {mentor.name
                          ? mentor.name
                              .split(" ")
                              .map((n) => n[0])
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-[#2e7d52]">
                        {mentor.title || mentor.specialization || "Tech Mentor"}
                      </p>
                      {mentor.company && (
                ...
                {/* Skills tags */}
                {mentor.skills && (Array.isArray(mentor.skills) ? mentor.skills.length > 0 : String(mentor.skills).trim().length > 0) && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {(Array.isArray(mentor.skills)
                      ? mentor.skills
                      : String(mentor.skills).split(",")
                    ).map((sk, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 rounded-md bg-[#DDF4E7]/60 text-[#2e7d52] border border-[#4CAF7D]/30 text-[10px] font-bold"
                      >
                        {String(sk).trim()}
                      </span>
                    ))}
